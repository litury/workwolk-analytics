...
├── hh.api.test.ts           # Интеграционные тесты HH.ru API
...
- **Цель**: Тестируют полные сценарии работы нашего API
- **Моки**: Нет моков, тестируем реальное приложение
- **Скорость**: Быстрые (Bun очень быстрый)
- **Требует**: Запущенный сервер (`bun run dev`)

### Интеграционные тесты (Integration Tests)
- **Файлы**: `tests/hh.api.test.ts`
- **Цель**: Тестируют интеграцию с внешним API (HH.ru)
- **Моки**: Нет моков, тестируем реальный внешний API
- **Требует**: Интернет-соединение

**Что тестируется в hh.api.test.ts (10 тестов):**

| Группа | Тесты |
|--------|-------|
| Доступность | API отвечает 200 |
| Структура вакансии | Обязательные поля, зарплата, работодатель |
| Поиск | Базовый поиск, фильтрация по запросу, пагинация |
| Детали вакансии | Получение по ID, description и key_skills |
| Справочники | Dictionaries возвращает experience, employment |

**Разница E2E vs Integration:**
- **E2E**: Тестирует наш код от входа до выхода (пользователь → API → БД → ответ)
- **Integration**: Тестирует взаимодействие нашего кода с внешними системами (HH.ru API)
